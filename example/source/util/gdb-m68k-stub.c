...
//mc68000 doesn't work in this stub...
#define mc68020
...
extern void stub_putDebugChar(char a);        /* write a single character      */
extern int stub_getDebugChar();        /* read and return a single char */
...
void returnFromException (Frame * frame) asm ("returnFromException");
void handle_exception (int exceptionVector) asm ("handle_exception");

#define FRAMESIZE 500
int   gdbFrameStack[FRAMESIZE];
static Frame *lastFrame asm ("lastFrame") =0;

/*
 * these should not be static cuz they can be used outside this module
 */
int registers[NUMREGBYTES/4] asm ("registers");
int superStack asm ("superStack") =0;

#define STACKSIZE 10000
int remcomStack[STACKSIZE/sizeof(int)];
int* stackPtr asm ("stackPtr") = &remcomStack[STACKSIZE/sizeof(int) - 1];
...
static ExceptionHook oldExceptionHook=0;
 ...
 */
const short exceptionSize[] asm("exceptionSize") = { 4,4,6,4,4,4,4,4,29,10,16,46,12,4,4,4 };
#endif
...
void return_to_super() asm ("return_to_super");
void return_to_user() asm ("return_to_user");

asm(" \n\
.text \n\
.globl return_to_super \n\
return_to_super: \n\
        movel   registers+60,%sp \n\
        movel   lastFrame,%a0 \n\
        bra     return_to_any \n\
                \n\
...
extern void _catchException() asm("_catchException");
...
\n\
        ...
        jbsr     returnFromException   /* jbsr, but never returns */\n\
");
#else /* mc68000 */
          ...
          if (checksum != xmitcsum)
            {
              ...
              stub_putDebugChar ('-');        /* failed checksum */
            }
          else
            {
              stub_putDebugChar ('+');        /* successful transfer */

              /* if a sequence char is present, reply the sequence ID */
              if (buffer[2] == ':')
                {
                  stub_putDebugChar (buffer[0]);
                  stub_putDebugChar (buffer[1]);
  ...
  do
    {
      ...
      while ((ch = buffer[count]))
        {
          stub_putDebugChar (ch);
          checksum += ch;
          count += 1;
        }

      stub_putDebugChar ('#');
      stub_putDebugChar (hexchars[checksum >> 4]);
      stub_putDebugChar (hexchars[checksum % 16]);

    }
  while (stub_getDebugChar () != '+');
      ...
      switch (*ptr++)
        ...
        case 'C':
          ...
          while (frame)
            {
              if (remote_debug)
                printf ("frame at 0x%p has pc=0x%x, except#=%d\n",
...
set_debug_traps ()
{
  extern void _debug_level7() asm("debug_level7");
  extern void remcomHandler() asm("remcomHandler");
  int exception;
  initializeRemcomErrorFrame ();
